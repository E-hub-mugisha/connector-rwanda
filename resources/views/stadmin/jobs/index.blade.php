        ...
        <div class="col-lg-12 grid-margin stretch-card">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="d-flex gap-1">
                                        <!-- Edit Button -->
                                        <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editJobModal{{ $job->id }}">
                                            Edit
                                        </button>
                                        <!-- View Button -->
                                        <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#viewJobModal{{ $job->id }}">
                                            View
                                        </button>
                                        <!-- Update Status Button -->
                                        <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#updateStatusModal{{ $job->id }}">
                                            Update Status
                                        </button>
                                        <!-- Job Applicants Button -->
                                        <a class="btn btn-primary" href="{{ route('provider.jobs.applications', $job->id) }}">
                                            Job Applicants <i class="fa fa-users"></i> {{ $job->applications->count() }}
                                        </a>
                                        <!-- Delete Button -->
                                        <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteJobModal{{ $job->id }}">
                                            Delete
                                        </button>
                                    </td>
                                </tr>
            ...
            <div class="modal fade" id="updateStatusModal{{ $job->id }}" tabindex="-1">
                <div class="modal-dialog">
                    <form action="{{ route('provider.jobs.updateStatus', $job->id) }}" method="POST" class="modal-content">
                        @csrf
                        @method('PUT')
            ...
            <div class="modal fade" id="editJobModal{{ $job->id }}" tabindex="-1">
                <div class="modal-dialog modal-lg">
                    <form action="{{ route('provider.jobs.update', $job->id) }}" method="POST" class="modal-content">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="modal-body">
                            @include('stadmin.jobs.form', ['job' => $job])
                        </div>
                        <div class="modal-footer">
            ...
            <div class="modal fade" id="deleteJobModal{{ $job->id }}" tabindex="-1">
                <div class="modal-dialog">
                    <form action="{{ route('provider.jobs.destroy', $job->id) }}" method="POST" class="modal-content">
                        @csrf
                        @method('DELETE')
            ...
            <div class="modal fade" id="addJobModal" tabindex="-1">
                <div class="modal-dialog modal-lg">
                    <form action="{{ route('provider.jobs.store') }}" method="POST" class="modal-content">
                        @csrf
                        ...
                        <div class="modal-body">
                            @include('stadmin.jobs.form', ['job' => null])
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
